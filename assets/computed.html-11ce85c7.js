import{_ as n,o as s,c as a,d as t}from"./app-87212aaa.js";const p="/blog/assets/computed-7d15fd01.png",o={},e=t(`<h1 id="computed处理" tabindex="-1"><a class="header-anchor" href="#computed处理" aria-hidden="true">#</a> computed处理</h1><p>处理<code>computed</code>相关的逻辑，发生在<code>initState</code>中，接下来我们详细分析与<code>computed</code>相关的逻辑。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">initState</span> <span class="token punctuation">(</span><span class="token parameter"><span class="token literal-property property">vm</span><span class="token operator">:</span> Component</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>因为此时的<code>total</code>值为<code>1</code>，所以会返回<code>this.firstName + this.lastName</code>的值，而<code>firstName</code>和<code>lastName</code>又是定义在<code>data</code>中的响应式变量，会依次触发<code>firstName</code>和<code>lastName</code>的<code>getter</code>，然后进行依赖收集。在组件渲染完毕后，<code>fullName</code>的依赖数组<code>subs</code>此时会有四个<code>watcher</code>，分别是三个计算属性<code>watcher</code>和一个渲染<code>watcher</code>。无论这三个计算属性<code>watcher</code>哪一个值更新了，都会再次重复以上的流程，这就是<code>computed</code>更新的过程。</p><p>在分析完<code>computed</code>的相关流程后，我们可以得到如下流程图</p><div style="text-align:center;"><img src="`+p+'"></div>',46),c=[e];function u(l,k){return s(),a("div",null,c)}const r=n(o,[["render",u],["__file","computed.html.vue"]]);export{r as default};
