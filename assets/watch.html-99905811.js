import{_ as n,o as a,c as s,d as p}from"./app-ef3230e2.js";const t={},o=p(`<h1 id="watch处理" tabindex="-1"><a class="header-anchor" href="#watch处理" aria-hidden="true">#</a> watch处理</h1><p>在介绍完处理<code>computed</code>相关的逻辑后，我们接下来看<code>watch</code>是如何处理的。</p><h2 id="watch初始化" tabindex="-1"><a class="header-anchor" href="#watch初始化" aria-hidden="true">#</a> watch初始化</h2><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">initState</span> <span class="token punctuation">(</span><span class="token parameter"><span class="token literal-property property">vm</span><span class="token operator">:</span> Component</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">)</span><span class="token operator">:</span> Function <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>我们可以发现，<code>$watch</code>方法主要做两件事情：<strong>创建Watcher实例</strong>和<strong>返回unwatchFn函数</strong>，接下来我们分别对这两部分的逻辑进行详细的解释。</p><h3 id="创建watcher实例" tabindex="-1"><a class="header-anchor" href="#创建watcher实例" aria-hidden="true">#</a> 创建Watcher实例</h3><p>我们先来看一下<code>Watcher</code>构造函数的代码：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token comment">// 精简代码</span>
<span class="token keyword">class</span> <span class="token class-name">Watcher</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>我们从构造函数中可以看到，当实例化一个<code>watch</code>的时候，会根据传递的<code>options</code>来处理<code>deep</code>、<code>user</code>、<code>lazy</code>、<code>sync</code>以及<code>before</code>属性。<code>watcher</code>根据不同的用法，有几种不同的分类：</p><ul><li><code>render watcher</code>：渲染<code>watcher</code>，例如当在<code>template</code>模板中使用<code>{{}}</code>语法读取一个变量的时候，此时这个变量收集的依赖就是<code>render watcher</code>，当这个变量值更新的时候会触发<code>render watcher</code>进行组件的重新渲染。是否为渲染<code>warcher</code>，使用构造函数参数<code>isRenderWatcher</code>为<code>true</code>进行区分。</li><li><code>computed watcher</code>：计算属性<code>watcher</code>，当我们在定义计算属性的时候，计算属性收集的依赖就是另外一个或者多个变量，当其中一个变量的值发生变量，就会触发计算属性重新进行求值。是否为计算属性<code>watcher</code>，使用<code>options.lazy</code>为<code>true</code>进行区分。</li><li><code>user watcher</code>：用户自定义<code>watcher</code>，多发生在<code>this.$watch</code>或者组件<code>watch</code>选择配置中，此时收集的依赖就是变量自身，当变量的值发生变化的时候，就会调用<code>watch</code>提供的回调函数。是否为用户自定义<code>watcher</code>，使用<code>options.user</code>为<code>true</code>进行区分。</li></ul><h3 id="返回unwatchfn函数" tabindex="-1"><a class="header-anchor" href="#返回unwatchfn函数" aria-hidden="true">#</a> 返回unwatchFn函数</h3><p>我们在构造函数中可以发现，它定义了一个<code>_watchers</code>变量，然后在每次实例化的时候，把自身添加到这个数组中，这样做的目的是为了方便清除依赖。在之前的介绍中，我们知道<code>$watch</code>返回了一个<code>unwatchFn</code>函数，它用来取消监听。接下来，我们看一下<code>teardown()</code>方法的具体实现。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token comment">// Watcher类精简代码</span>
<span class="token keyword">class</span> <span class="token class-name">Watcher</span> <span class="token punctuation">{</span>
...
<span class="token keyword">class</span> <span class="token class-name">Dep</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p><code>teardown()</code>方法的实现很简单，就是从<code>deps</code>数组中移除当前的<code>watcher</code>，其中<code>deps</code>存储的是<code>Dep</code>实例。</p>`,31),e=[o];function c(l,u){return a(),s("div",null,e)}const r=n(t,[["render",c],["__file","watch.html.vue"]]);export{r as default};
