...
</code></pre></div><p>最后在继承完毕后，还处理了<code>props</code>、<code>computed</code>以及各种全局API方法，这部分的逻辑跟之前我们提到过的是一样的，不再此累述。</p>`,26),i=n("strong",null,"安装组件钩子函数",-1),d=n("code",null,"Vue",-1),m=n("code",null,"DOM",-1),y={href:"https://github.com/snabbdom/snabbdom",target:"_blank",rel:"noopener noreferrer"},g=n("code",null,"VNode",-1),f=a('<p><img src="'+l+`" alt="VNode钩子函数"></p><p>在<code>Vue</code>中也用到了这些钩子函数，它的定义如下：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> componentVNodeHooks <span class="token operator">=</span> <span class="token punctuation">{</span>
...
<span class="token keyword">const</span> vnode <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>在<code>createComponent</code>这一小节，我们介绍了组件会进行<code>mergeOptions</code>配置合并，为了更好的理解<code>path</code>的过程，我们会在接下来的小节优先介绍<code>mergeOptions</code>配置合并策略。</p>`,10);function v(b,w){const p=o("ExternalLinkIcon");return e(),c("div",null,[r,n("ul",null,[n("li",null,[i,s("：我们在前面提到过，"),d,s("中的虚拟"),m,s("借鉴了开源库"),n("a",y,[s("snabbdom"),u(p)]),s("的实现，在这个库里面当"),g,s("节点处于不同的场景下，提供了对应的钩子函数来方便我们处理相关的逻辑，这些钩子函数如下：")])]),f])}const x=t(k,[["render",v],["__file","createComponent.html.vue"]]);export{x as default};
