import{_ as o,r as e,o as c,c as l,a as n,b as s,e as p,d as t}from"./app-c9f86c19.js";const u={},i=t(`<h1 id="compiletofunctions" tabindex="-1"><a class="header-anchor" href="#compiletofunctions" aria-hidden="true">#</a> compileToFunctions</h1><h2 id="compile核心方法" tabindex="-1"><a class="header-anchor" href="#compile核心方法" aria-hidden="true">#</a> compile核心方法</h2><p>我们知道，在<code>runtime + compiler</code>的版本中，<code>$mount</code>方法和<code>Vue.compile</code>全局<code>API</code>都用到了<code>compileToFunctions</code>方法。在<code>Web</code>平台下，它是从<code>src/platforms/web/compiler/index.js</code>文件中引入的，代码如下：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> baseOptions <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">&#39;./options&#39;</span>
...
<span class="token punctuation">)</span><span class="token operator">:</span> CompiledResult <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="代码分析" tabindex="-1"><a class="header-anchor" href="#代码分析" aria-hidden="true">#</a> 代码分析</h2><p>在介绍完核心<code>compile</code>方法后，我们来分析一下<code>compileToFunctions</code>的实现逻辑：</p>`,19),r=n("strong",null,"CSP限制",-1),k=n("code",null,"CSP",-1),d={href:"https://developer.mozilla.org/zh-CN/docs/Web/HTTP/CSP",target:"_blank",rel:"noopener noreferrer"},m=n("code",null,"CSP",-1),y=n("blockquote",null,[n("p",null,"一个策略由一系列策略指令所组成，每个策略指令都描述了一个针对某个特定类型资源以及生效范围的策略。你的策略应当包含一个default-src策略指令，在其他资源类型没有符合自己的策略时应用该策略(有关完整列表查看default-src )。一个策略可以包含 default-src 或者 script-src 指令来防止内联脚本运行, 并杜绝eval()的使用。 一个策略也可包含一个 default-src 或 style-src 指令去限制来自一个 style 元素或者style属性的內联样式。")],-1),g=n("code",null,"CSP",-1),f={href:"https://developers.google.com/web/fundamentals/security/csp/#if_you_absolutely_must_use_it",target:"_blank",rel:"noopener noreferrer"},w=n("code",null,"CSP",-1),v=n("strong",null,"注意",-1),h=n("code",null,"Vue",-1),C=n("code",null,"1.0+",-1),j=n("code",null,"CSP",-1),F={href:"https://github.com/vuejs/vue/tree/1.0-csp",target:"_blank",rel:"noopener noreferrer"},_=t(`<p>根据以上描述，如果存在某些<code>CSP</code>限制，那么我们可能无法使用<code>text-to-JavaScript</code>机制，也就是说下面这些代码可能无法正常运行：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> func <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">Function</span><span class="token punctuation">(</span><span class="token string">&#39;return 1&#39;</span><span class="token punctuation">)</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>当再次编译<code>App</code>组件的时候，发现在<code>cache</code>对象中已经存在这个键，因此直接返回。</p>`,24);function b(x,O){const a=e("ExternalLinkIcon");return c(),l("div",null,[i,n("ul",null,[n("li",null,[r,s("："),k,s("是指内容安全策略，我们可以在"),n("a",d,[s("MDN"),p(a)]),s("上看到它对"),m,s("的定义、描述以及一些示例，同时我们可以看到它有下面这样一段描述：")])]),y,n("p",null,[s("如果想了解"),g,s("，你可以点击"),n("a",f,[s("Content Security Policy"),p(a)]),s("去深入学习有关"),w,s("方面的知识。")]),n("p",null,[v,s("："),h,s("只在"),C,s("提供了特定的"),j,s("兼容版本，你可以在"),n("a",F,[s("Vue Github"),p(a)]),s("分支仓库去查看这个版本的源代码。")]),_])}const S=o(u,[["render",b],["__file","compileToFunctions.html.vue"]]);export{S as default};
