...
## 自定义指令
除了我们之前提到过默认的全局默认指令`v-show`、`v-model`以外，我们还可以选择注册自定义指令。就像在**指令的注册和使用**小结中一样，我们注册的`v-focus`指令就是自定义指令。
### 钩子函数
...
```
我们可以在`unbindDirectives`方法的定义中看到，其方法内部调用了`updateDirectives`并且给方法的第二个参数传递了一个`emptyNode`空节点来实现对指令的解绑。
